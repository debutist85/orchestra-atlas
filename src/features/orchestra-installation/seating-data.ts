...
// Rear players retain their section order on the shared outer ring.
export const rearArcNodeIds = [
  'violin2-5-1', 'violin2-5-inserted-1', 'violin2-5-3', 'violin2-5-5',
  'viola-5-2', 'viola-5-3', 'viola-5-4',
] as const

// First-ring slots from left to right; no hidden positions.
export const firstRingSlots = [
  'violin1-1-1', 'violin1-1-2', 'violin2-1-1', 'violin2-1-2',
  'viola-1-1', 'viola-1-2', 'cello-1-1', 'cello-1-2',
] as const

// Second-ring slots; preserve the gap left by removed node 47.
export const secondRingSlots = [
  'violin1-2-1', 'violin1-2-2', 'violin1-2-3',
  'violin2-2-1', 'violin2-2-3', 'violin2-2-inserted-1',
  'viola-2-1', 'viola-2-3', null, 'cello-2-2', 'cello-2-3',
] as const

// Third-ring slots; preserve gaps for removed players without shifting neighbors.
export const thirdRingSlots = [
  'violin1-3-1', null, null, 'violin1-3-4',
  'violin2-3-1', null, 'violin2-3-3', 'violin2-3-4',
  'viola-3-1', 'viola-3-2', 'viola-3-3', 'viola-3-4',
  'cello-3-1', 'cello-3-2', null, 'cello-3-4',
] as const

// Fourth-ring slots from left to right. Nulls reserve removed/hidden players
// between visible nodes 8/9, 9/10, and 40/41 without changing live numbering.
export const fourthRingSlots = [
  'violin1-4-1', null, 'violin1-4-3', null, 'violin1-4-5',
  'violin2-4-1', 'violin2-4-2', 'violin2-4-3', 'violin2-4-4',
  'strings-ring-4-bridge', 'viola-4-2', 'viola-4-3', null, 'viola-4-5',
  'cello-4-1', 'cello-4-2', 'cello-4-3', 'cello-4-4', 'cello-4-5',
] as const

// Ring membership and angular order, independent of visible numbering.
// Add/remove a ring here together with its radius/range in config.ts.
export const seatingRingSlots: readonly (readonly (string | null)[])[] = [
  firstRingSlots, secondRingSlots, thirdRingSlots, fourthRingSlots,
  ['violin1-5-3', 'violin1-5-5', ...rearArcNodeIds],
]

// Authored coordinates retain their broad footprint. Trailing numbers below
// are legacy references; live numbering follows the filtered array order.
const authoredNodes: readonly OrchestraNodeDefinition[] = [
...
// Explicitly distinguish the established outer bass row without changing stable
// IDs used by palettes/debugging. Non-string nodes keep their assigned families.
export const orchestraNodes: readonly OrchestraNodeDefinition[] = authoredNodes.map(node => {
  if (node.sectionId !== 'strings') return node
  const stringInstrument = node.id.startsWith('cello-4-') ? 'doubleBass'
    : node.id.startsWith('cello-') ? 'cello'
    : node.id.startsWith('viola-') ? 'viola'
    : node.id.startsWith('violin2-') ? 'violin2' : 'violin1'
  return { ...node, stringInstrument }
})
