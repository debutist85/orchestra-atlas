...
Stem URLs are the file map in `src/features/listening/excerpt.ts`: `/audio/beethoven-7th-2nd/opus/{file}` under `public/audio`. Numbered parts (flute-1, flute-2) mix into one instrument channel. Instruments without a file (trombone, tuba, harp, celesta, extra percussion) have no channel. Rights are not inferred. Missing files leave that channel out; if none load, playback stays unavailable.

Instrument activity is a precomputed envelope in `public/activity/{excerpt}.json`, generated by `npm run audio:activity -- <excerpt-id>` from the WAV masters. Runtime lookup is `currentTime / sampleInterval` — it does not read PCM or use AnalyserNode. Playback and activity stay separate: the profile describes the musical timeline; transport playing/paused decides whether that lookup is audible. The previous live RMS analyzer remains in `instrument-activity.ts` and can be re-enabled with `useLiveInstrumentAnalysis` for comparison.

Master WAV stems are encoded to Opus with `npm run audio:encode`. See `docs/audio-assets.md`. Playback loads those Opus files; decoding still expands each stem to PCM in memory.
